...
- **Test-retest occasions.** The framework can audit that two evaluation artifacts have identical benchmark hash, endorsement config, and paraphrase variant. It cannot audit that they were produced on different occasions; it can record run timestamps and provider response IDs where available, but the analyst's commitment to actually running re-evaluations on different occasions is the substantive piece. Honesty in producing two independent runs is the audit.
...
The right response to these is to organise the research community to do the work, not to extend the tool to pretend otherwise. The framework's posture: make the work cheap enough to do and expensive enough to skip. Every workflow requirement has *infrastructure* that makes the work cheaper and *declarations* that make its presence/absence visible in the report.

## The workflow

The methodology runs in nine phases. The framework handles phases 1–7; phases 0, 4 (in part), and 8 are research-program work the framework supports but cannot do for you. The framework's *cheap-to-do-it-right, expensive-to-cut-corners* asymmetry shows up at every phase: skipping a phase is fine; doing it without declaring you skipped it is not. The report verdict refuses to render ✅ "defensible" when the skips are not at peace with the declared scope.
...
The methodology requires two runs for R22. The reliability check is what lets the headline κ_C be interpreted as signal rather than as noise from a single draw.
...
The output prints the test-retest κ, the flip count, the per-item flips (capped at 20 with the full list in the JSON), and a one-sentence stability verdict. Stable means κ ≥ 0.8; moderately stable means ≥ 0.6; substantively unstable means < 0.6 — at which point the headline κ_C cannot be interpreted as signal under that reliability level.
...
> **Caveat on self-report fields.** `held_out_items_used` and `training_data_separation_verified` are currently self-report: the framework does not cross-check them against the per-item `construction_metadata` that should substantiate them. `test_retest_run` is self-report in a different sense — the framework can audit that you supplied a retest artifact and that it has the right shape, and the verdict-gate cap will catch the case where the supplied artifact shows substantive instability, but the framework cannot audit that the two runs were produced on genuinely different occasions. Your honesty in setting these booleans is the substantive audit.
...
| R22 reliability of the measurement | Full (within-run + across-run); research-program (different occasions) | `VerdictDistribution`, `AggregateDispersion`, Politis-Romano CIs via `--ci`; `infereval retest`; verdict-gate at scope ≥ `domain_D_as_sampled` |
...
The framework's job is to make the construct-validity work *cheap enough to do* and *expensive enough to skip*. The infrastructure moves in both directions: cheaper because the framework handles the bookkeeping (factorial validation, paraphrase cycling, structural checks, dispersion surfacing, retest comparison, report generation); more expensive to skip because skipping requires explicitly opting out of checks the framework otherwise runs by default. The asymmetry plays out at three concrete places:
...
infereval retest out/eta-A.json out/eta-B.json \
  --benchmark benchmark.json -o out/retest.json
