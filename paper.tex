...
\begin{remark}[Openness]
\label{rem:openness}
Containment is the only structural constraint imposed on $I_M$. The derived frame may be nonmonotonic, nontransitive, or non-contractive; whether it is so for a given $M$ is an empirical property of $\mu_M$, not something the construction forces or forbids.
\end{remark}

\begin{remark}[Parameters]
\label{rem:parameters}
The frame $I_M$ depends on $B$, $k$, and the expression relation $\mathrm{Expr}$. Different admissible choices yield different but equally well-formed derived frames. The structural properties recorded below hold for any admissible choice.
\end{remark}
...
The classical core holds for any $M$ and any admissible parameters; it is a property of the NMMS-extended consequence relation, not of $I_M$. The material base may be substructural while the extension carries a classical logical core.
