...
The construction also surfaces commitments the inferentialist program has tended to leave implicit. Most notably: that content-attribution to a system is relative to a carving of the domain into bearers supplied by an analyst, i.e., a participant in the discursive practice associated with the domain; that the choice between characterizing $M$'s generative and evaluative behavior is a substantive philosophical commitment, not a technicality; and that mastery-claims about M are claims about structural features of M's inferential behavior, made empirically tractable through agreement with analyst-supplied benchmarks. These commitments are not novel to the LLM case --- they are arguably present in the inferentialist program from the start --- but the LLM case forces them into view. We return to these points in the discussion.
...
% We return in Remark \ref{rem:operational} to how this asymmetry can be addressed at the structural level of the derived frame rather than at the level of individual endorsements.
\end{remark}
...
% \begin{remark}[The structural character of mastery claims]
% \label{rem:operational}
% Once the benchmark is made explicit as a set of analyst-labeled implications $\beta$, mastery-claims about $M$ become empirically tractable: the coverage and kappa measures of an evaluation $\eta$ give measurable proxies for $M$'s participation in the inferential practice the benchmark records. We resist, however, the deflationary thesis that mastery-claims about $M$ reduce to such measurements. The Hlobil--Brandom framework defines rich structure on a frame $\langle B, I_M \rangle$ --- implicational roles $R(\alpha)$, content inclusion, ranges of subjunctive robustness, intersubstitutability \emph{salva consequentia} --- and mastery-claims are most naturally read as claims about these structural features. Benchmark agreement is one window onto that structure: a finite, analyst-mediated projection of a richer fact about $M$'s inferential behavior. The methodology gives tractable purchase on the projection; the structure itself is what mastery-claims target.

% This structural reading also clarifies the relationship between the methodology and \emph{in-principle} claims about classes of systems, of the kind Simonelli advances when he argues that pure LLMs trained on linguistic data could in principle possess all concepts \cite{simonelli2026}. An in-principle claim of this form is not a claim about what any particular $M$ actually does on any particular $\beta$; it is a claim about the space of frames $\langle B, I_M \rangle$ that systems of a given architectural class can realize. Empirical measurements bear on such claims in two ways. First, they constrain the in-principle claim from below: if some pure LLM $M$ realizes a frame with mastery-grade structural features on a sufficiently rich benchmark, then pure LLMs \emph{can} realize such frames, and the in-principle claim about the class is to that extent vindicated. Second, persistent and systematic gaps in measurements across many models and benchmarks constitute defeasible evidence against the in-principle claim, since architectural or training improvements within the class might close such gaps. The methodology cannot settle in-principle claims directly; convergence of empirical measurements toward the inter-analyst baseline $\kappa^*_F(\beta)$, accompanied by stabilization of structural features of derived frames across enrichments of $B$ and $\beta$, is the form evidence for in-principle claims takes here.

% A subtlety remains. Even reconstructed in this way, the in-principle claim is necessarily indexed to the analytic resources the framework supplies: the strongest form the methodology can bear on is the claim that for any analyst-recognizable domain admitting a carving into bearers and a benchmark of inferential examples, pure LLMs in principle can realize frames that approach analyst-baseline agreement on that benchmark, with the structural features the framework treats as constitutive of mastery on that carving. This is weaker than the unrestricted in-principle claim that pure LLMs can possess \emph{all} concepts, but it is the form content-attribution takes once the carving-relativity flagged in Remark~\ref{rem:analyst} is taken seriously: on the global strong inferentialist view, content-attribution is already relative to a carving, and in-principle claims about content-possession inherit that relativity. The methodology does not weaken Simonelli's claim so much as articulate the form a defensible version of it takes within the framework his argument presupposes.

% What the methodology cannot do is settle, on its own, whether the characterizations it produces capture anything about $M$ in a sense stronger than structural features of $\langle B, I_M\rangle$ as accessed through analyst-mediated measurement. Whether better choices of $B$, $\delta$, $\mathrm{ctx}_\Gamma$, $\mathrm{ctx}_\Delta$, and $\beta$ yield characterizations that converge on a stable account of M's inferential behavior is an empirical and methodological question the construction supports but does not answer. This is, we suggest, the empirically tractable form the inferentialist program takes when applied to systems whose participation in discursive practice is asymmetric and analyst-mediated. The program does not lose its content; it loses the pretense that the analyst's role can remain implicit. What replaces the pretense is a methodology in which the analyst's role is explicit at every stage --- in the carving of bearers, the construction of contexts, the production of benchmark verdicts coherent with discursive practice, and the choice of thresholds at which agreement counts as mastery --- and in which mastery-claims are understood as claims about structural features of $M$'s inferential behavior that the methodology renders empirically accessible without reducing to the access itself.
% \end{remark}

\begin{remark}[Carving-relativity and the form of in-principle claims]
\label{rem:operational}
The methodology inherits a relativity the framework already contains: content-attribution proceeds relative to an analyst-supplied carving of the practice into bearers, expression functions, and benchmark verdicts (Remark~\ref{rem:analyst}). Simonelli's in-principle claim that pure LLMs can possess all concepts \cite{simonelli2026} does not survive this relativity in its unrestricted form. What survives is a carving-indexed claim: for any analyst-recognizable domain admitting a carving into bearers and a benchmark of inferential examples, pure LLMs can in principle realize frames that approach analyst-baseline agreement $\kappa^*_F(\beta)$ on that benchmark, with the structural features the framework treats as constitutive of mastery on that carving. This is a genuine weakening, and one the global strong inferentialist should accept as the price of the framework's commitments: on the global strong inferentialist view, content-attribution is already relative to a carving, and in-principle claims about content-possession inherit that relativity.
\end{remark}
...
Simonelli, following his stop sign dialogue with GPT-4.1, states ``There is, to be sure, a way to more systematically test this sort of material inferential knowledge, and developing and conducting such a test is likely a worthwhile project'' \cite[p.~19]{simonelli2026}. Though Simonelli takes his anecdotal evidence to suggest robust mastery, the systematic test he flags as worthwhile is precisely what the present methodology supplies. With it, mastery of a material implication becomes something that can be measured against a specified benchmark, with results decomposable by implication and by variation type. Differences between models become measurable as differences in coverage and kappa across these decompositions rather than as differences in aggregate score. The agreement of an LLM's evaluative behavior with the inferential practice articulated by a community of analysts becomes characterizable as the construction's measurements specify it, relative to the benchmark and the carving the analysts supply. The relativity is not merely methodological: in-principle claims about LLM concept-possession inherit it, taking the carving-indexed form developed in Remark \ref{rem:operational} rather than the unrestricted form in which they are sometimes stated. And the question of what role LLMs can play in the game of giving and asking for reasons --- a question that has tended to be treated as a matter for conceptual settlement --- becomes one a research program can productively pursue.
...
What the methodology cannot do is settle, on its own, the extent to which structural features of $\langle B, I_M \rangle$, as accessed through analyst-mediated measurement, capture $M$'s inferential behavior in full. Whether better carvings, richer benchmarks, and more comprehensive measurement converge on characterizations that vindicate mastery-attributions at the level of structure is a question the present construction makes empirically pursuable rather than philosophically settled. The construction does not resolve the question; it changes its form, from one to be settled by conceptual analysis to one a research program can take up.
